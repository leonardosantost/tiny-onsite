...
serve(async (request) => {
  try {
    ...
    searchUrl.searchParams.set('user_product_id', userProductId)
    ...
    if (!ids.length) {
      return new Response(JSON.stringify({ user_product_id: userProductId, items: [] }), {
        status: 200,
        headers: { 'Content-Type': 'application/json', ...corsHeaders() },
      })
    }
    ...
    console.error('ml-user-product error', e)
